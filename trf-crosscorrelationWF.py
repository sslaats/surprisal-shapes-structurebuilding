...
info = GA['bottomup_splitWF_surprisal']['trf'].info
...
_, clusters, pvals, _ = perm['frequency_surprisal']
...
#ch_idx = list(range(0,269)) # all channels

# copy the GA and take away the edge artifact
high = GA['bottomup_splitWF_surprisal']['trf']['frequency_high_surprisal'].copy()
high = high.crop(tmin=-0.05, tmax=0.95)

low = GA['bottomup_splitWF_surprisal']['trf']['frequency_low_surprisal'].copy()
...
for ch in range(len(ch_idx)):
    x1 = high_channels[ch, :] / high_channels[ch, :].std()
    x2 = low_channels[ch,:] / low_channels[ch, :].std()
...
axes[0,0].set_title('Word frequency response')
...
sns.despine()
plt.tight_layout()
fig.savefig(os.path.join(resultsdir, 'correlation-analysis-WF.svg'))
