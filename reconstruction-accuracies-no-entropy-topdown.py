...
# %% load the stats ...
with open(os.path.join(resultsdir, 'ra_stats.pkl'), 'rb') as f:
    rastats = pickle.load(f)
...
for syntype in ['bottomup']:
    for distype in ['surprisal']:
        ...
        if len(sensitive_sensors_idx) > 0:
            ...
            randomsplit = np.asarray(r_data.loc[r_data['model'] == f'{syntype}_split_random', 'r_values']).reshape((len(set(r_data['subject'])),269))[:,sensitive_sensors_idx]
            onesplit = np.asarray(r_data.loc[r_data['model'] == f'{syntype}_split_{distype}', 'r_values']).reshape((len(set(r_data['subject'])),269))[:,sensitive_sensors_idx]
...
for stattype in ['surprisal']:
    ...
    for syntype in ['bottomup']:
        ...
        #  get data into shape
        X1 = np.asarray(r_data.loc[r_data['model'] == f'{syntype}_split_random', 'r_values'])
        X1 = X1.reshape((len(set(r_data['subject'])),269))
        
        X2 = np.asarray(r_data.loc[r_data['model'] == f'{syntype}_split_{stattype}', 'r_values'])
...
plt.tight_layout()
fig.savefig(f'{resultsdir}/random_split.svg')
